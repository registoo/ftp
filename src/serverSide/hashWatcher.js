...
module.exports = async function(serveDir) {
  ...
  fs.watch(serveDir, { recursive: true }, (eventype, filename) => {
    ...
    async function func(ENOENT) {
      ...
      if (ENOENT) {
        neDB.remove({ fileName: fullFilePath }, err => {
          if (err) {
            console.log(
              `Ошибка при удалении документа из neDB в файле ${__filename}:\r\n${err}`
            );
            return;
          }
      ...
      neDB.find({ fileName: fullFilePath }, function(err, doc) {
        ...
        if (doc.hash !== currentHash)
          neDB.update(
            ...
            err => {
              if (err) {
                console.log(
                  `Ошибка при замене документа neDB в файле ${__filename}:\r\n${err}`
                );
                return;
              }
